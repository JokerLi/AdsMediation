...
//            return config.cache_time * 1000;
	...
	public static void setDownloadServiceState(boolean isResume){
		SharedPreferences sp = CMAdManager.getContext().getSharedPreferences("market_request_download_service", Context.MODE_PRIVATE);
    ...
    private static void ensureSp() {
        if(null == mSp){
            mSp = CMAdManager.getContext().getSharedPreferences("market_config", Context.MODE_PRIVATE);
        }
    }
    ...
    public static void initUserAgent(){
        if(TextUtils.isEmpty(sUserAgent)) {
            try {
                sUserAgent = MarketConfig.getString(MarketConfig.USER_AGENT, "");
                if(TextUtils.isEmpty(sUserAgent)) {
                    sUserAgent = AdWebViewUtils.getUserAgentString(CMAdManager.getContext());
