...
//                freshPicksConfig();
    ...
    public static CMBaseFactory createFactory() {
        if (sAdFactory == null) {
            try {
                Class className = Class.forName("com.cmcm.adsdk.CMAdManagerFactory");
                if (sAdFactory == null) {
    ...
    public static void addRenderAdapter(String loaderKey, CMNativeAdTemplate.ICMNativeAdViewAdapter adapter){
        CMBaseFactory adFactory = createFactory();
        if (adFactory != null) {
