...
public class PerferenceUtil {
	...
	private static String spfName = String.format("%s_%s", "cmadsdk", CMAdManager.getMid());;
	...
	public synchronized static String getCacheJsonStr(String defValue){
		try{
			if(sSharePreference == null) {
				sSharePreference = CMAdManager.getContext().getSharedPreferences(spfName, Context.MODE_PRIVATE);
			}
