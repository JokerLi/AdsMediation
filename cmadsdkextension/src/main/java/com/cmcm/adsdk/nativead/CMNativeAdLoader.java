...
public class CMNativeAdLoader extends CMBaseNativeloaderAdapter implements NativeloaderAdapter.NativeAdapterListener, INativeAd.IAdOnClickListener, LifeCycleDelegate{
    ...
    private Map<String, Object> getLoadExtras(int num, String placementId){
        ...
        extras.put(CMBaseNativeAd.KEY_JUHE_POSID, mPositionId);
        extras.put(CMBaseNativeAd.KEY_PLACEMENT_ID, placementId);
        extras.put(CMBaseNativeAd.KEY_LOAD_SIZE, num);

        extras.put(CMBaseNativeAd.KEY_RCV_REPORT_RES, mInternalNativeLoader.getReportRes(getAdTypeName()));
//        extras.put(CMBaseNativeAd.KEY_PEG_REPORT_RES, mInternalNativeLoader.getReportRes(NativeloaderAdapter.RES_TYPE_PEG, getAdTypeName()));

        extras.put(CMBaseNativeAd.KEY_REPORT_PKGNAME, mInternalNativeLoader.getReportPkgName(getAdTypeName()));
//        long cacheTime = CMAdManager.getAdTypeCacheTime(mInternalNativeLoader.getAdKeyType());
        ...
        extras.put(CMBaseNativeAd.KEY_CACHE_TIME, defaultCacheTime);
        if(requestParams != null) {
            if(requestParams instanceof CMBannerParams){
                extras.put(CMBaseNativeAd.KEY_BANNER_VIEW_SIZE, ((CMBannerParams) requestParams).getCMBannerAdSize());
            }
            extras.put(CMBaseNativeAd.KEY_CHECK_VIEW, !requestParams.getReportShowIgnoreView());
            extras.put(CMBaseNativeAd.KEY_FILTER_ADMOB_INSTALL_AD, requestParams.isFilterAdmobInstallAd());
            extras.put(CMBaseNativeAd.KEY_FILTER_ADMOB_CONTENT_AD, requestParams.isFilterAdmobContentAd());
            extras.put(CMBaseNativeAd.KEY_EXTRA_OBJECT, requestParams.getExtraObject());
            extras.put(CMBaseNativeAd.KEY_TAB_ID, requestParams.getTabId());
            extras.put(CMBaseNativeAd.KEY_IS_TOP, requestParams.getIsTop());
        }else {
            extras.put(CMBaseNativeAd.KEY_CHECK_VIEW, true);
        }
        extras.put(CMBaseNativeAd.KEY_IS_FEED, mIsFeed);
        extras.put(CMBaseNativeAd.KEY_IS_ORIONAD, isOrionAd());
        return extras;
    }
    ...
    void appendAd(INativeAd ad) {
        ...
        mLocalExtras.put(CMBaseNativeAd.KEY_AD_TYPE_NAME, getAdTypeName());
        CMNativeAd cMNativeAd = new CMNativeAd(mContext, this, mLocalExtras, (CMBaseNativeAd)ad);
    ...
    private void recordClick(INativeAd nativeAd){
        ...
        String rawString = "";
        try {
            CMNativeAd ad = (CMNativeAd) nativeAd;
        ...
        String placementID = (String) mLocalExtras.get(CMBaseNativeAd.KEY_PLACEMENT_ID);
