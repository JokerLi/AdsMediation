...
public class FeedListAdManager {
    ...
    private List<CMNativeAd> mDuplAdCache;//重复广告缓存池
    ...
    public FeedListAdManager(@NonNull final Context context,
        ...
        this(new ArrayList<CMNativeAd>(cacheSize));
    ...
    private FeedListAdManager(@NonNull final List<CMNativeAd> nativeAdCache) {
        mOrionBrandCache = new HashMap<String, List<CMNativeAd>>();
        ...
        mDuplAdCache = new ArrayList<CMNativeAd>();
    ...
    private void initParams() {
        mINativeAdLoaderListener = new INativeAdLoaderListener() {
            ...
            public void adFailedToLoad(int errorcode) {
                ...
                map.put(NET_WORK_TYPE, String.valueOf(NetworkUtil.getNetworkState(CMAdManager.getContext())));
    ...
    private boolean saveNativeAd(final INativeAd nativeAd){
        if(nativeAd == null || !(nativeAd instanceof CMNativeAd)){
        ...
        if(mNativeAdCache.size() == 0){
            if(TextUtils.isEmpty(mLastAdTitle) || !mLastAdTitle.equals(nativeAd.getAdTitle()) || !mIsFilterDuplAd){
                addAd2Cache((CMNativeAd)nativeAd);
                ...
                if (null != ad && ad instanceof CMNativeAd) {
    ...
    private void addAd2Cache(final CMNativeAd nativeAd){
        ...
        if(Const.KEY_OB.equals(nativeAd.getAdTypeName())){
            ...
            List<CMNativeAd> list = mOrionBrandCache.get(nativeAd.getTypeId());
            if(list == null){
                list = new ArrayList<CMNativeAd>();
            }
            list.add(nativeAd);
    ...
    private INativeAd getAdFromOrionBrandCache(INativeAd ad, String type){
        if(ad.getAdTypeName().startsWith(Const.KEY_OB)){
            List<CMNativeAd> list = mOrionBrandCache.get(type);
            isAdCanUse(list);
    ...
    private boolean isNotNeedRemoveOb(){
        ...
        if(null != keys && keys.size() > 0){
            for(String key : keys){
                List<CMNativeAd> list =  mOrionBrandCache.get(key);
    ...
    private boolean isAdCanUse(List<CMNativeAd> list){
        if(list != null && list.size() > 0){
            Iterator<CMNativeAd> i = list.iterator();
            while (i.hasNext()){
                CMNativeAd ad = i.next();
                if(ad.hasExpired()){
                    i.remove();
                }
            }
            ...
            return ThreadHelper.runOnUiThreadBlockingNoException(new Callable<INativeAd>() {
                ...
                public INativeAd call() throws Exception {
                    ...
                    if(mNativeAdCache.size() <= 0){
                        ...
                        Iterator<CMNativeAd> iterator = mNativeAdCache.iterator();
                    ...
                    if(mDuplAdCache.size() <= 0){
                        ...
                        Iterator<CMNativeAd> mDupleAditerator = mDuplAdCache.iterator();
    ...
    private void addAd2DupleCache(INativeAd tempAd){
        if(null != tempAd && tempAd instanceof CMNativeAd) {
            Logger.i(TAG, "addAd2DupleCache before size :" + mDuplAdCache.size());
            mDuplAdCache.add((CMNativeAd)tempAd);
