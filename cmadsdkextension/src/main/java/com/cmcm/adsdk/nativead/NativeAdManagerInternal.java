...
public class NativeAdManagerInternal implements INativeReqeustCallBack, LifeCycleDelegate, INativeAd.IAdOnClickListener {
    ...
    */
    protected void checkIfAllfinished() {
        ...
        //先判断是否有超级强量
        if(mIsOpenPriority) {
            CMNativeAdLoader picksLoader = mLoaderMap.getAdLoader(Const.KEY_CM);
    ...
    protected boolean isAllLoaderFinished() {
        ...
        if ( mLoadingStatus.getWaitingBeansNumber() == 0 ) {
            for (PosBean posBean : mConfigBeans) {
                String adTypeName = posBean.getAdName();
                CMNativeAdLoader loader = mLoaderMap.getAdLoader(adTypeName);
    ...
    public void onPause() {
        ...
        for(PosBean posBean : mConfigBeans){
            CMNativeAdLoader loader = mLoaderMap.getAdLoader(posBean.name);
            if(loader != null){
                loader.onPause();
            }
        }
